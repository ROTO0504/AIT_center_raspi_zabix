...
| `web_dev.py` | Web UI 開発用ローカルサーバ（GPIO・Graph なしでモックデータ表示） |
...
| `.gitea/workflows/deploy.yml` | Gitea Actions による自動デプロイ定義 |
...
- 障害復旧時 → LED・ブザーの通知なし（状態のみ更新）
- 起動時 → 緑 LED 点滅
- ボタン押下 → LED 消灯 + ブザー 1 回（ステータスは維持）
- ステータス判定に一致しないメールは WARN ログに記録
- Web ダッシュボード: `http://<ラズパイIP>:8080`
  - 「動作中バージョン」項目で現在動いているコミットハッシュを表示(デプロイ反映確認用)
  - ログセクションは折りたたみ可能（`<details>` で初期は展開）
- ログ表示はアラート検出・判定更新時のみ（ポーリング毎の定常ログなし）

## Web UI ローカル開発

ラズパイや Azure 認証なしで UI だけを確認したい場合は `web_dev.py` を使う。
GPIO・Microsoft Graph には接続せず、モックデータを 10 秒周期で切り替えて表示する（normal → high → unknown）。

```bash
.venv/bin/python web_dev.py
```

- ブラウザ: `http://localhost:8080/`
- ポート変更: `WEB_PORT=8081 .venv/bin/python web_dev.py`
- HTML テンプレートは `web_app.py` の `HTML` を共有しているため、UI 修正はそちらを編集する
